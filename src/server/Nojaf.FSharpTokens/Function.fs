...
module GetTokens =
    ...
    let private toJson (tokens: Token list) =
        ...
        tokens
        |> List.map (Thoth.Json.Net.Encode.Auto.generateEncoder<Token> (false, extra))
        |> Encode.list
        |> Encode.toString 4
...
[<FunctionName("Tokens")>]
let Run([<HttpTrigger(AuthorizationLevel.Anonymous, "get", "post", Route = "{*any}")>] req (*, log: ILogger*) : HttpRequest) =
